...
class GowerMetricTest(unittest.TestCase):
    ...
    def test_cat_nom(self):
        data = np.array([
            ['Private', 'Assoc-acdm', 'Never-married', 'Sales', 'Not-in-family', 'Black', 'Male', 'United-States'],
            ['Private', 'Assoc-voc', 'Married-civ-spouse', 'Craft-repair', 'Husband', 'Asian-Pac-Islander', 'Male','?']
        ])
        gower = GowerMetric(np.array([DataType.CATEGORICAL_NOMINAL for _ in range(data.shape[1])]))
        gower.fit(data)
    ...
    def test_ratio(self):
        ...
        gower = GowerMetric(np.array([DataType.RATIO_SCALE for _ in range(data.shape[1])]))
        gower.fit(data)
        res = gower(data[0], data[1])
    ...
    def test_bin_asym(self):
        ...
        gower = GowerMetric(np.array([DataType.BINARY_ASYMMETRIC for _ in range(data.shape[1])]))
        gower.fit(data)
        res = gower(data[0], data[1])
    ...
    def test_weights(self):
        ...
        gower = GowerMetric(np.array([DataType.RATIO_SCALE for _ in range(data.shape[1])]), weights=weights)
        gower.fit(data)
        res = gower(data[0], data[1])
