...
class GowerMetric:
    ...
    def __call__(
        ...
        cat_nom_bin_sym_cols = (
            self.dtypes == DataType.CATEGORICAL_NOMINAL
        ) | (self.dtypes == DataType.BINARY_SYMMETRIC)
        ratio_cols = self.dtypes == DataType.RATIO_SCALE
        bin_asym_cols = self.dtypes == DataType.BINARY_ASYMMETRIC

        section_time_1 += timeit.default_timer() - start
        ...
        distances = [
            ...
            * dist_func(
                ...
                vector_1[cat_nom_bin_sym_cols],
                vector_2[cat_nom_bin_sym_cols],
            )
            if cat_nom_bin_sym_cols[0].size
            else [],
            ...
            * dist_func_ratio(
                ...
                vector_1[ratio_cols],
                vector_2[ratio_cols],
                self.ranges_[ratio_cols],
                self.h_[ratio_cols],
            )
            if ratio_cols[0].size
            else [],
            self.weights_[bin_asym_cols]
            * dist_func(
                self._bin_asym,
                vector_1[bin_asym_cols],
                vector_2[bin_asym_cols],
            )
            if bin_asym_cols[0].size
            else [],
        ]
    ...
    def _ratio_scale(
        ...
        global section_time_2, section_time_3
        ...
        zeros = absolute <= h
        section_time_2 += timeit.default_timer() - start
        ...
        if vector_1.size == 1:
            ...
            section_time_3 += timeit.default_timer() - start
...
class GowerMetric2:
    ...
    def __call__(
        ...
        # distances = [
        #     self.weights_[cat_nom_bin_sym_cols] *
        #     np.vectorize(dist_func)(self._cat_nom,
        #                             vector_1[cat_nom_bin_sym_cols],
        #                             vector_2[cat_nom_bin_sym_cols]) if cat_nom_bin_sym_cols[0].size else [],
        #     self.weights_[ratio_cols] *
        #     np.vectorize(dist_func_ratio)(self._ratio_scale,
        #                                   vector_1[ratio_cols],
        #                                   vector_2[ratio_cols],
        #                                   self.ranges_[ratio_cols],
        #                                   self.h_[ratio_cols]) if ratio_cols[0].size else [],
        #     np.vectorize(dist_func)(self._bin_asym,
        #                             vector_1[bin_asym_cols],
        #                             vector_2[bin_asym_cols]) if bin_asym_cols[0].size else []
        # ]
        # distances = np.concatenate(distances)
...
if __name__ == "__main__":
    ...
    print(section_time_1)
    print(section_time_2)
    print(section_time_3)
